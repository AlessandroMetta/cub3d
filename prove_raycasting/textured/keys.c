...
static int		movement(t_vars *vars)
{
	if(vars->keyUp == 1)	// W
	{
		if(ft_strchr("0NWES", map[(int)(vars->posX + vars->dirX * MOVSPEED)][(int)(vars->posY)]))
			vars->posX = vars->posX + vars->dirX * MOVSPEED;
		if(ft_strchr("0NWES", map[(int)(vars->posX)][(int)(vars->posY + vars->dirY * MOVSPEED)]))
	...
	if(vars->keyLeft == 1)	// A
	{
		if(ft_strchr("0NWES", map[(int)(vars->posX + vars->planeX * MOVSPEED)][(int)(vars->posY)]))
			vars->posX = vars->posX + vars->planeX * MOVSPEED;
		if(ft_strchr("0NWES", map[(int)(vars->posX)][(int)(vars->posY + vars->planeY * MOVSPEED)]))
	...
	if(vars->keyUp == 1 || vars->keyDown == 1 || vars->keyLeft == 1 || vars->keyRight == 1)
		return (1);
	return (0);
}

static  int		rotation(t_vars *vars)
{
...
{
	int ret1 = movement(vars);
	int ret2 = rotation(vars);
	if (ret1 || ret2)
		return (1);
	return (0);
}
