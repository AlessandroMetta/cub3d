...
int	render_next_frame(t_vars *vars)
{
	...
	game.addr = mlx_get_data_addr(game.img, &game.bbp, &game.ll, &game.end);
	...
	while(vsop < screenWidth)
	{
		...
		drawTexture(game, vars, vsop, drawStart, drawEnd);
		drawEnviroment(game, vars, vsop, drawStart, drawEnd);
		vsop++;
	}
